...
=======
	double res=- ( 2.0*q1 ) / ( pow( ( ( 4.0*q1*q1 ) + ( eps*eps ) ) , 3.0/2.0 ) ) ;
>>>>>>> 0a05c40c9ca918dc110c88a0bb19b2c8b98508a3
	return res;
}
double dp2(double q1, double q2)
{
...
double runge_kutta(double h, int N, double p1_in, double p2_in, double q1_in, double q2_in)
{
	...
	for(int i=1;i<N;i++)
	{
		...
		q1k2=dq1(p1[i-1]+( 0.5 * h * p1k1 ));
		q2k2=dq2(p2[i-1]+( 0.5 * h * p2k1 ));
		p1k2=dp1(q1[i-1]+( 0.5 * h * q1k1 ));
		p2k2=dp2(q1[i-1]+( 0.5 * h * q1k1 ),q2[i-1]+(  0.5 * h * q2k1 ));

		q1k3=dq1(p1[i-1]+( 0.5 * h * p1k2 ));
		q2k3=dq2(p2[i-1]+( 0.5 * h * p2k2 ));
		p1k3=dp1(q1[i-1]+( 0.5 * h * q1k2 ));
		p2k3=dp2(q1[i-1]+( 0.5 * h * q1k2 ),q2[i-1]+( 0.5 * h * q2k2 ));
		
		q1k4=dq1(p1[i-1]+( h * p1k3 ));
		q2k4=dq2(p2[i-1]+( h * p2k3 ));
		p1k4=dp1(q1[i-1]+( h * q1k3 ));
		p2k4=dp2(q1[i-1]+( h * q1k3 ),q2[i-1]+( h * q2k3 ));
